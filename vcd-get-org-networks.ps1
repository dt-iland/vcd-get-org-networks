...
# Purpose: Retrieve all networks from a specified vCloud Director organization
# https://github.com/dt-iland/vcd-get-org-networks
...
# 1. Create API session using requested API version
# 2. Use query service to locate Org
# 3. Extract Org ID
# 4. Retrieve administrative view of Org using extracted ID
# 5. Extract all OrgNetwork records
# 6. Generate URN for all OrgNetwork records and append the attribute/key pair
# 7. Generate report
...
# Section for api/query?type=organization

# Update $OutFile
$OutFile = $QueryOrg + "_get_api_query_organization_$ApiVersion.txt"
Write-Host("OutFile = $OutFile")

# URI for organization list
$AdminOrgListURI = $BaseURI + "api/query?type=organization&pageSize=$QueryPageSize&filter=(name==$QueryOrg)"

# Capture task start time
$TaskTime = Get-Date

# Execute organization REST GET query
Write-Host("REST GET query: $AdminOrgListURI") -ForegroundColor Yellow
$RestResponse = ExecuteRest -RestURI $AdminOrgListURI -RestMethod Get -HttpHeaders $SessionHeaders -DisableKeepAlive $true -ExpectedCode 200 -FailMessage "An error occurred while attempting to execute the REST GET query!" -OutFile $OutFile -ThrowOnFail $ThrowOnRestFailure
...
$ResponseContent =
@"
---BEGIN REST XML RESPONSE from ${AdminOrgListURI}---
${AsciiXml}
---END REST XML RESPONSE from ${AdminOrgListURI}---
"@
...
# Use XPath to get the admin URI of the specified organization
$AdminOrgURI = $($XmlResponseBody.SelectSingleNode("/vcd:QueryResultRecords/vcd:OrgRecord[@name='$QueryOrg']/@href", $XmlNSM)).Value
Write-Host("$QueryOrg admin URI: $AdminOrgURI") -ForegroundColor Yellow

# Use regular expression to extract the ID of the Org
$AdminOrgID = [regex]::Match($AdminOrgURI, "(?<=\/api\/org\/).*").Captures[0].Value
Write-Host("$QueryOrg ID: $AdminOrgID") -ForegroundColor Yellow

# Generate URN for Org
$AdminOrgURN = "urn:vcloud:org:$AdminOrgID"
Write-Host("$QueryOrg URN: $AdminOrgURN") -ForegroundColor Yellow

# Section for api/org/{id}

# Update $OutFile
$OutFile = $QueryOrg + "_get_api_org_" + $AdminOrgID + "_$ApiVersion.txt"
Write-Host("OutFile = $OutFile")
...
# Execute organization REST GET query
Write-Host("REST GET query: $AdminOrgURI") -ForegroundColor Yellow
Write-Host("NOTE: Response can take up to 1min+ depending on the number of Org VDC networks") -ForegroundColor Yellow
$RestResponse = ExecuteRest -RestURI $AdminOrgURI -RestMethod Get -HttpHeaders $SessionHeaders -DisableKeepAlive $true -ExpectedCode 200 -FailMessage "An error occurred while attempting to execute the REST GET query!" -OutFile $OutFile -ThrowOnFail $ThrowOnRestFailure
...
$ResponseContent =
@"
---BEGIN REST XML RESPONSE from ${AdminOrgURI}---
${AsciiXml}
---END REST XML RESPONSE from ${AdminOrgURI}---
"@
...
# Use XPath to get all vnd.vmware.vcloud.orgNetwork results
$AdminOrgNetworks = $XmlResponseBody.SelectNodes("/vcd:Org/vcd:Link[@type='application/vnd.vmware.vcloud.orgNetwork+xml']", $XmlNSM)

# Display total number of Org VDC networks
Write-Host("$QueryOrg has $($AdminOrgNetworks.Count) Org VDC networks") -ForegroundColor Yellow

# Array to hold report records
$AdminOrgNetworkReport = @()

# Extract the attribute/value pairs from the XML nodes and add them to the report array
foreach ($AON in $AdminOrgNetworks) {
